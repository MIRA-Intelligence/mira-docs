...
不齐全的字段会被 [guardrail](usage/ui/guardrail-and-auto-repair) 捕获并触发自动修复回合。
...
UI 上看到“暂停推进新实验、出现缺失字段提示”，通常就是 guardrail 在工作。详见 [Guardrail 与自动修复](usage/ui/guardrail-and-auto-repair)。
...
- **Provider**：模型供应商（OpenAI、Anthropic、OpenRouter、DeepSeek、Ollama……）。`{{PROJECT_CORE_NAME}}` 内置 [20+ provider](usage/agent-config/providers-and-runtime)。
- **Model**：具体型号（如 `anthropic/claude-opus-4-5`）。
- **Routing**：开启 `routeByComplexity` 后，`{{PROJECT_CORE_NAME}}` 会按任务复杂度路由到 `smallModel` / `mediumModel` / `largeModel`，便宜的活走便宜的模型。详见 [模型路由](usage/agent-config/model-router)。
...
完整列表见 [Skills 与 Tools](usage/agent-config/skills-and-tools)。
...
详见 [Skills 与 Tools](usage/agent-config/skills-and-tools)。
